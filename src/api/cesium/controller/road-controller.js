...
let viewer; // Store the viewer object
let roadEntities = []; // Store road entities
// 动画相关
let animationPath;
let animationModel;
let animationPoints = [];
let animationDataObject;
 ...
 */
const MODEL_LIST = [
  { id: 'air', name: 'Air', uri: './static/models/Cesium_Air.glb', scale: 1 },
  { id: 's1000rr', name: 'S1000RR', uri: './static/models/s1000rr.glb', scale: 0.01 },
  { id: 'r1200gs', name: 'R1200GS', uri: './static/models/r1200gs.glb', scale: 0.5 },
  { id: 'drone', name: 'Drone', uri: './static/models/CesiumDrone.glb', scale: 1 },
 ...
 * 非贴地模型配置
 */
const NO_CLAMP_TO_GROUND_MODEL = {
  air: { height: 5000 },
  drone: { height: 100 },
  oh_58d: { height: 1000 }
}
 ...
 * 批量添加线路
 */
function addRoads(roadsData, viewerParam) {
  if (!viewer) {
    viewer = viewerParam; // Store the viewer object if not already stored
  }
...
function zoomToRoad(roadCoordinates) {
  ...
  viewer.camera.flyToBoundingSphere(boundingSphere, {
    ...
    offset: new Cesium.HeadingPitchRange(0, -Math.PI / 4, boundingSphere.radius * 2), // Adjust the camera offset as needed
  });
}
 ...
 * 播放线路轨迹动画
 */
function playRoadAnimation(roadData) {
  if (!viewer) {
    viewer = MapController.getViewer()
  }
  clearAnimation();
  const { speed = 100, coordinates, model_id = 'air' } = roadData;
  // const roadPointData = JSON.parse(
  ...
  // );
  const roadPointData = coordinates;

  // meassure line length
  let totalLength = 0;
  // store distance for each point
  let distanceArray = [];
  for (let i = 1; i < roadPointData.length; i++) {
    const prevPoint = roadPointData[i - 1];
    const currPoint = roadPointData[i];
    const distance = Cesium.Cartesian3.distance(
      Cesium.Cartesian3.fromDegrees(prevPoint.longitude, prevPoint.latitude, prevPoint.height),
      Cesium.Cartesian3.fromDegrees(currPoint.longitude, currPoint.latitude, currPoint.height)
    );
    distanceArray.push(distance);
    totalLength += distance;
  }
  // distance / (speed * 1000) / 60 / 60 = time (in seconds)

  const totalSeconds = totalLength / (speed * 1000) * 60 * 60;
  ...
  const noClampToGroundModel = NO_CLAMP_TO_GROUND_MODEL[roadData.model_id];
  const noClampToGround = noClampToGroundModel !== undefined;


  for (let i = 0; i < roadPointData.length; i++) {
    const dataPoint = roadPointData[i];
    const distanceArr = distanceArray.slice(0, i)
    const distanceCurr = distanceArr.reduce((acc, cur) => acc + cur, 0);
    const timeStepInSeconds = distanceCurr / (speed * 1000) * 60 * 60;
    const time = Cesium.JulianDate.addSeconds(start, timeStepInSeconds, new Cesium.JulianDate());
    const position = Cesium.Cartesian3.fromDegrees(dataPoint.longitude, dataPoint.latitude, noClampToGround ? noClampToGroundModel.height : dataPoint.height);
    // add to linePositions
    linePositions.push(position);
    // Store the position along with its timestamp.
    // Here we add the positions all upfront, but these can be added at run-time as samples are received from a server.
    positionProperty.addSample(time, position);
    // const animationPoint = viewer.entities.add({
  ...
  animationPath = viewer.entities.add({
    // clampToGround line
    polyline: {
      show: noClampToGround ? false : true,
      positions: linePositions,
      // clampToGround: noClampToGround ? false : true,
      clampToGround: true,
      width: 6,
  ...
  async function loadModel() {
    ...
    // const airplaneUri = './static/models/Cesium_Air.glb';
    // const airplaneUri = './static/models/r1200gs.glb';
    // const airplaneUri = './static/models/s1000rr.glb';
    const modelData = MODEL_LIST.find(({ id }) => id === model_id);
    const { uri, scale } = modelData
    const airplaneEntity = viewer.entities.add({
      ...
      model: {
        ...
        scale: scale,
        maximumScale: 100,
        heightReference: noClampToGround ? Cesium.HeightReference.NONE : Cesium.HeightReference.CLAMP_TO_GROUND,
      },
      // Automatically compute the orientation from the position.
      orientation: new Cesium.VelocityOrientationProperty(positionProperty),
      // real points
      path: new Cesium.PathGraphics({ width: 5, show: false }),
      // Cesium.TrackingReferenceFrame.AUTODETECT  INERTIAL  VELOCITY  ENU
      // trackingReferenceFrame: noClampToGround ? Cesium.TrackingReferenceFrame.VELOCITY : Cesium.TrackingReferenceFrame.ENU,
      trackingReferenceFrame: Cesium.TrackingReferenceFrame.ENU,
      // viewFrom: new Cesium.Cartesian3(-30, -60.0, 45.0)
    });
...
function clearAnimation() {
  if (animationPath) {
    viewer.entities.remove(animationPath)
    animationPath = undefined
  }
  if (animationModel) {
    viewer.entities.remove(animationModel)
    animationModel = undefined
  }
 ...
 * 根据 ID 检查当前动画对象是否一致
 */
function checkAnimationObjectById(id) {
  if (animationDataObject && animationDataObject.id === id) {
    clearAnimation();
  }
}
